...
public class ProcExplorer {
    private static final String TAG = ProcExplorer.class.getSimpleName();

    private final Context mContext;

    public ProcExplorer(Context context) {
        mContext = context;
    }

    public String handleCommand(InetSocketAddress inetSocketAddress) {
        byte[] sourceIp = inetSocketAddress.getAddress().getAddress();
...
//                + ":" + ((destinationPort >= 0) ? Integer.toHexString(destinationPort) : "any"));
    ...
    private static String getAppInfoForUid(Context context, int uid)
        ...
        for (String packageName : packageNames) {
            ...
            result.append(packageManager.getApplicationLabel(ai) + " (" + packageInfo.packageName + ")"); //+ " " + packageInfo.versionCode
        }
